<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
    <CategoryCard v-for="category in categories" :key="category.id" :category="category" @view="handleViewCategory" @edit="handleEditCategory" @delete="handleDeleteCategory" />
  ...
  <div v-if="editingCategory" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
      ...
      <CategoryForm :category="editingCategory" @submit="handleUpdateCategory" @cancel="editingCategory = null" />
    </div>
  </div>

  <!--  deleting modal -->
  <div v-if="deletingCategory" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
      <h2 class="text-2xl font-bold text-gray-900 mb-4">Delete Category</h2>
      <p class="text-gray-600 mb-6">
        Are you sure you want to delete this category?
        <span class="font-semibold text-red-600">All tasks in this category will also be deleted.</span>
        This action cannot be undone.
      </p>
      <div class="flex gap-3">
        <button @click="confirmDelete" class="flex-1 bg-red-500 text-white font-semibold py-3 rounded-lg hover:bg-red-600 transition-colors">Delete</button>
        <button @click="deletingCategory = null" class="flex-1 bg-gray-200 text-gray-700 font-semibold py-3 rounded-lg hover:bg-gray-300 transition-colors">Cancel</button>
      </div>
    </div>
  </div>
</template>
...
const emit = defineEmits(["add-category", "update-category", "delete-category"]);
...
const confirmDelete = () => {
  if (deletingCategory.value) {
    emit("delete-category", deletingCategory.value);
